...
import chapter25.exercise01_02.AbstractTree;
...
public class BST<E extends Comparable<E>> extends AbstractTree<E> {
    ...
    public TreeNode<E> getNode(TreeNode<E> node, E element) {
        if (node == null)
            return null;
        if (node.element.compareTo(element) == 0)
            return node;
        TreeNode<E> left = getNode(node.left, element);
        TreeNode<E> right = getNode(node.right, element);
    ...
    protected boolean isLeaf(TreeNode<E> node, E element) {
        if (node == null)
            return false;
        if (node.left == null && node.right == null && node.element.compareTo(element) == 0)
            return true;
        return isLeaf(node.left, element) || isLeaf(node.right, element);
    }
